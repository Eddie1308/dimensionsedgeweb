...
export function ClientForm({ initial }: { initial: ClientFormValue }) {
  ...
  function update<K extends keyof ClientFormValue>(k: K, val: ClientFormValue[K]) {
    setV((p) => ({ ...p, [k]: val }));
  }
  ...
  async function onDelete() {
    if (!isEdit || !confirm("Delete this client?")) return;
    setStatus("deleting");
    try {
      const res = await fetch(`/api/admin/clients/${initial.id}`, { method: "DELETE" });
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      <FormSection title="Identity">
        <FormGrid>
          ...
          <Field label="Industry (AR)" htmlFor="industryAr">
            <input id="industryAr" dir="rtl" value={v.industryAr} onChange={(e) => update("industryAr", e.target.value)} className={inputStyles} />
          </Field>
        </FormGrid>
      </FormSection>
      ...
      <FormSection title="Logo">
        {v.logoUrl ? (
          <div className="flex items-start gap-4">
            ...
            <button type="button" onClick={() => update("logoUrl", "")} className="text-xs text-[var(--color-brand-700)] hover:underline">Remove</button>
          </div>
        ) : (
